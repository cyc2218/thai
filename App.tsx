        ...
        <header className="pt-8 pb-3 px-6 flex justify-between items-center bg-transparent z-10">
          <div className="flex flex-col">
            <h1 className="text-2xl font-black tracking-tighter text-[#5D5443] flex items-center gap-2 drop-shadow-sm">
              <span className="text-3xl">🇹🇭</span> Changsis Thai trip
            </h1>
          ...
          <div className="flex gap-2">
            ...
            <button 
              ...
              className={`w-12 h-12 rounded-2xl border-2 mori-shadow flex items-center justify-center transition-all active:scale-90 ${
                isEditMode ? 'bg-[#8BAE8E] border-[#8BAE8E] text-white rotate-0' : 'bg-white border-[#E0E5D5] text-gray-300 rotate-3'
              }`}
            >
          ...
          <div className="max-w-md mx-auto flex justify-around items-center h-16">
            {[...NAV_ITEMS, { id: 'members', label: '成員', icon: <span>👥</span> }].map((item) => (
              <button
                ...
                className={`flex flex-col items-center justify-center flex-1 transition-all duration-300 active:scale-75 ${
                  activeTab === item.id ? 'text-[#8BAE8E] -translate-y-2' : 'text-gray-300'
                }`}
              >
